...
const useAreaSuggestions = (
  ...
  const fetchSuggestions = useCallback(
    (input) => {
      ...
      if (!freeSearch && (!selectedCity || !selectedState)) {
        setSuggestions([]);
        return;
      }
      ...
      debounceTimer.current = setTimeout(() => {
        ...
        const countryName =
          selectedCountry?.label?.replace(/^\S+\s/, "") ?? "India";
        ...
        service.getPlacePredictions(
          ...
          (preds, status) => {
            ...
            const filtered = preds.filter((p) => {
              ...
              if (!desc.includes(cityLower) || !desc.includes(stateLower))
                return false;
              const cityIdx = desc.indexOf(cityLower);
              const areaPart =
                cityIdx > 0
                  ? desc.substring(0, cityIdx).replace(/,\s*$/, "").trim()
                  : "";
              return (
                areaPart.startsWith(inputLower) || areaPart.includes(inputLower)
              );
            });
...
const LocationSelector = ({ onChange, errors }) => {
  ...
  useEffect(() => {
    ...
    if (india) {
      ...
      if (defaultStates.length > 0)
        setSelectedState({
          value: defaultStates[0].isoCode,
          label: defaultStates[0].name,
        });
    }
  }, []);
  ...
  useEffect(() => {
    onChange?.({
      country: selectedCountry?.value ?? "IN",
  ...
  const handleSelectSuggestion = (prediction) => {
    ...
    placesServiceRef.current.getDetails(
      ...
      (place, status) => {
        ...
        const cityMatch =
          cityComp?.toLowerCase() === selectedCity?.value?.toLowerCase() ||
          cityComp?.toLowerCase() === selectedCity?.label?.toLowerCase();
        ...
        if (!countryMatch || !stateMatch || !cityMatch) {
          setAreaError(
            `"${cityComp || "This area"}" does not belong to ${selectedCity?.label}, ${selectedState?.label}. Please select a valid area.`,
          );
          setQuery("");
          setDropdownOpen(false);
          setSuggestions([]);
        ...
        setAreas((prev) =>
          prev.find((a) => a.name === newArea.name) ? prev : [...prev, newArea],
        );
        setAreaError("");
        setQuery("");
        setDropdownOpen(false);
  ...
  options={states}
  value={selectedState}
  onChange={(value) => { setSelectedState(value); setSelectedCity(null); setAreas([]); setStateInput(""); }}
...
/>
        {hasError("state") && (
          <div className="field-error">State is required</div>
        )}
      </div>
  ...
  options={cities}
  value={selectedCity}
  onChange={(value) => { setSelectedCity(value); setAreas([]); setCityInput(""); }}
...
/>
      ...
      >
        ...
        <input
          ...
          placeholder={
            !selectedCity
              ? "Select a city first"
              : freeSearch
                ? "Search any area (filter removed)..."
        ...
        {dropdownOpen && suggestions.length > 0 && (
          ...
          >
            {suggestions.map((pred) => {
              const cityIdx = pred.description
                .toLowerCase()
                .indexOf((selectedCity?.label ?? "").toLowerCase());
              const areaLabel =
                !freeSearch && cityIdx > 0
        ...
        {hasError("areas") && (
          <div className="field-error">At least one area is required</div>
        )}
